...
def run(t_acc,results,latent_path):
    print(latent_path)
...
if __name__ == "__main__":
    ...
    for latent_path in tqdm(glob.glob(latent_all)):
        tasks.append(pool.submit(run, t_acc=t_acc,results=results,latent_path=latent_path))

    print('>>>>>>>>>> PROCESS >>>>>>>>>>')
    pbar = tqdm(as_completed(tasks), total=len(tasks))
    for t in pbar:
        effective = t.result()
        total += effective
        pbar.set_description_str(f'Last WSI gives {effective}/{total} patches')
